...
const setupModal = () => {
    ...
    document.querySelectorAll('.btn-tx-details').forEach(btn => btn.addEventListener('click', openModal));
    ...
    if (btnExchange) {
        btnExchange.addEventListener('click', () => {
            ...
            setTimeout(() => {
                ...
                state.balances[swap.to]   = (state.balances[swap.to]   || 0) + recAmt;

                updateWalletDisplay();

                // сбрасываем инпут и пересчитываем с новым балансом
                document.getElementById('swap-input-give').value = '1';
                renderSwapUI();

                showToast(`${giveAmt} ${swap.from} → ${recAmt.toFixed(2)} ${swap.to}`, 'success');
